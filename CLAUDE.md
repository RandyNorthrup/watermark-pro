...
- `src/client/routeTree.gen.ts` is generated by the router plugin on every
  dev/build/test run and committed, so a fresh checkout lints and type-checks
  before anything has been built; CI diffs it after the build.
