...
| Camera metadata (GPS, capture time) in exports or shared files | Every export is re-encoded from pixels by the canvas; the source EXIF block is never copied, the orientation tag is applied to the pixels first, and there is no keep-metadata option                            | `bulk.browser.test.ts` ("writes no metadata")              |
