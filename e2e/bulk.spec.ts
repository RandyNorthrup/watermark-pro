...
test('preserves folder paths, names outputs by size, and reports a one-photo override honestly', async ({
  ...
  const fileNames = page.getByLabel('File names', { exact: true })
  await fileNames.fill(' '.repeat(3))
  await expect(fileNames).toHaveAttribute('aria-invalid', 'true')
  await expect(fileNames).toHaveAccessibleDescription(/The pattern must produce a name\./)
  await fileNames.fill('{name}-{width}x{height}')
  await expect(fileNames).toHaveAttribute('aria-invalid', 'false')
  await expect(fileNames).not.toHaveAccessibleDescription(/The pattern must produce a name\./)
