...
test('exports a recoverable invisible PNG mark and distinguishes absent and unreadable images', async ({
  ...
  await page.getByRole('combobox', { name: 'Saved Watermark', exact: true }).selectOption(preset.id)
  await page.getByRole('tab', { name: 'Saved', exact: true }).click()
  ...
  await expect(page.getByRole('combobox', { name: 'Format', exact: true })).toHaveText('PNG')
  await expect(invisible).toBeEnabled()
