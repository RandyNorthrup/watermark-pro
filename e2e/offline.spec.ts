...
import { createWorkspace, expect, expectAccessible, navigateTo, saveSampleImage } from './support'
...
test('keeps offline presets and photo saves across reload and reconnect', async ({
  ...
  await page.getByLabel('Watermark name').fill('Offline base')
  ...
  await page.getByLabel('Watermark name').fill('Created offline')
  await page.getByRole('button', { name: 'Save watermark' }).click()
  ...
  await expect(sync.panel.getByText('Synchronized', { exact: true })).toBeVisible()
  await sync.close()
...
test('preserves both versions of a conflicting offline preset edit', async ({
  ...
  await page.getByLabel('Watermark name').fill('Original preset')
  await page.getByRole('button', { name: 'Save watermark' }).click()
  ...
  await expect
    ...
    .toBe(1)
  await expectOfflineStatus(page, 'Synchronized')
  ...
  await expect(page.getByLabel('Watermark name')).toHaveValue('Original preset')
  await waitForOfflineReadiness(page)
  const observer = await offlineNetwork.createObserver()
  await offlineNetwork.setOffline(true)
  await page.getByLabel('Watermark name').fill('Local version')
  ...
  await expect(conflict.panel.getByText(/This watermark changed elsewhere/)).toBeVisible()
  await expectAccessible(page)
  await conflict.panel.getByRole('button', { name: 'Keep both versions' }).click()
  await expect(conflict.panel.getByText('Synchronized', { exact: true })).toBeVisible()
