...
beforeAll(async () => {
  ...
  expect(await services.users.promoteToPlatformAdmin(ownerEmail)).toBe(true)
})

describe('single anchored administrator over D1 and real authentication', () => {
  it('anchors the first administrator and refuses every second-admin API entry point', async () => {
    expect(await services.accounts.siteOwnerId()).toBe(ownerId)
    expect(await services.users.promoteToPlatformAdmin(memberEmail)).toBe(false)
    for (const [path, body] of [
      ['/api/auth/admin/set-role', { userId: memberId, role: 'admin' }],
      ['/api/auth/admin/set-role', { userId: memberId, role: ['user', 'admin'] }],
      ['/api/auth/admin/update-user', { userId: memberId, data: { role: 'admin' } }],
      [
    ...
    expect(users.filter((row) => row.role === 'admin').map((row) => row.id)).toEqual([ownerId])
  ...
  it('enforces the same singleton directly in D1, including mixed roles and anchor changes', async () => {
    await expect(
      services.db.update(user).set({ role: 'admin' }).where(eq(user.id, memberId)),
    ).rejects.toThrow()
    await expect(
