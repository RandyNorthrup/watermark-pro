...
Workers (a `*.workers.dev` subdomain, no custom domain), and is built to
enterprise-grade security standards with role-based access control.
...
| R15 | Hosted on the owner's Cloudflare account, no custom domain              | M0, M8                               |
...
| A6  | Transactional email goes through **Cloudflare Email Sending** (the `send_email` binding) behind a provider interface. A `console` provider exists for local development only and the app refuses to start with it in production.                                                        | Chosen by the owner on 2026-09-05. Requires a domain in the Cloudflare account for the sender address; see Q7.                                                                                                                            |
...
| A11 | The `workers.dev` subdomain is acceptable as the only origin for cookies and share links.                                                                                                                                                                                               | Stated by the owner.                                                                                                                                                                                                                      |
...
| Q7  | **Email needs a domain.** Cloudflare Email Sending can only send from a domain that is a zone in your Cloudflare account. The app itself has no custom domain (R15), but the sender address needs one. Is there an existing domain in the account to send from, or should a low-cost domain be added purely for email (the app would still be served from `workers.dev`)?                       | M1 ships with the `console` email provider for local development. Verification and password-reset emails cannot go out in production until a sending domain exists. |
...
- The `workers.dev` origin is
  shared with other Cloudflare users' Workers at the parent domain level. Cookies
  are scoped to the full subdomain, so this is a phishing/branding concern rather
  than a cookie-isolation one.
...
- **Goal:** production deployment on `workers.dev`, admin console, hardening, documentation.
- **Scope:** admin console (users, orgs, bans, audit log viewer), Turnstile on sign-up, rate limit tuning, CSP report-only → enforce review, threat model document, dependency review, `wrangler deploy` from CI on tags, runbook.
