...
| A6  | Transactional email goes through **Cloudflare Email Sending** (the `send_email` binding) from `no-reply@blowmoney.net`, behind a provider interface. A `console` provider exists for local development and tests; the app refuses to start with it in production. | Provider and sender confirmed by the owner on 2026-09-05. Email Sending is included in the Workers Paid plan.                                                                             |
