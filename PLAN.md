...
| Q10 | Tag-driven deploys need repository secrets: `.github/workflows/deploy.yml` reads `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`. Until they are added in the GitHub repository settings, a `v*` tag runs the gates and fails at the deploy step.                                                                                                                                                                                                                                                                      | Deploy from the workstation with `npm run deploy` (unaffected).                               |
