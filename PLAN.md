...
| Q10 | Tag-driven deploys need `CLOUDFLARE_API_TOKEN` in the repository secrets (`CLOUDFLARE_ACCOUNT_ID` was added 2026-09-06). Creating an API token needs the dashboard: the wrangler OAuth session has no token-management scope. Until it exists, a `v*` tag runs the gates and fails at the deploy step.                                                                                                                                                                                                                    | Deploy from the workstation with `npm run deploy` (unaffected).                               |
