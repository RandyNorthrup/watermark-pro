...
| R15 | Hosted on the owner's Cloudflare account at **lumafoil.com** (updated 2026-09-05; the brief originally said no custom domain) | M0 config, M8 certification          |
...
| Primitives      | Radix UI (`radix-ui` unified package), lucide-react icons                                         | 1.6.7 / 1.40.0                                  | React 19 satisfies lucide-react's declared peer range; 1.40.0 was reviewed from the publisher's 2026-09-03 release before the final M19 refresh.                                                                                                                                                                                                                                                            |
...
| UI kit          | Radix UI unified package, lucide-react, class-variance-authority, clsx, tailwind-merge            | 1.6.7 / 1.40.0 / 0.7.1 / 2.1.1 / 3.6.0          | Lucide 1.40.0 adds icons and metadata without removing the named icons used here; the final build and UI matrix verify the exact refreshed pin.                                                                                                                                                                                                                                                             |
| Fonts           | `@fontsource-variable/inter` (self-hosted)                                                        | 5.3.0                                           | OFL licence; no third-party font origin, so `font-src 'self'` holds.                                                                                                                                                                                                                                                                                                                                        |
| Test tooling    | `@testing-library/user-event`, `@better-auth/core` (drift test only), lighthouse, chrome-launcher | 14.6.7 / 1.7.2 / 13.4.1 / 1.2.1                 | `@better-auth/core` matches Better Auth. user-event 14.6.7 retains the installed DOM peer and fixes keyboard/focus behavior. chrome-launcher 1.2.1 changes its default RenderDocument flag, so the final owned-page and browser audits are required on this pin.                                                                                                                                            |
...
as affected by GHSA-rgj7-g3m4-5g8c. The root override pins `sharp` **0.35.4**
without replacing Cloudflare's tooling or weakening the audit. Verification:
`npm info sharp@0.35.4 peerDependencies engines version --json` reports no
peers and Node >=20.9.0 (host: 24.20.0); `npm info sharp time --json` reports
publication on 2026-08-26, older than the seven-day release guard. This is a
patch within the same sharp minor. Remove the override when upstream pins a
safe version; full workerd/browser tests must verify this override before
certification.

The same refresh pins the deprecated `@esbuild-kit/core-utils` loader's nested
`esbuild` to **0.25.12**, which drizzle-kit 0.31.10 already uses directly. This
removes the separate vulnerable 0.18.20 copy (GHSA-67mh-4wv8-2f99) without
changing Vite or Wrangler's esbuild 0.28.1 copies. The loader declares `^0.18.20`,
so this is an explicit transitive compatibility override; `npm info
esbuild@0.25.12 peerDependencies engines version --json` reports no peers and
Node >=18. Its use is validated by `npm run db:generate`, the Worker tests and
the full build. Remove this override when drizzle-kit drops its legacy loader.

M19 creative proof and brand tooling (2026-09-08): `sharp` **0.35.4** is also
an exact direct development dependency for reproducible rasterization of the
canonical SVG brand assets and real product screenshots. `jsqr` **1.4.0**
(Apache-2.0) is an exact development-only dependency for independent decoding
of exported QR watermarks, rather than checking the encoder against itself.
`npm info sharp@0.35.4 engines peerDependencies --json` reports Node >=20.9.0
and no peers; `npm info jsqr@1.4.0 license peerDependencies --json` reports
Apache-2.0 and no peers. The installed Node 24.20.0 satisfies these ranges.
No runtime dependency was added for the 500 extra font families or 400 Fluent
stickers: versioned WOFF2/SVG assets, individual notices, hashes and provenance
are checked into the existing static-asset path. Fonttools 4.64.0 is local-only
art tooling under ignored `temp/brand-tools`; no global installation was made.

Public-identifier scanner review (2026-09-08): Gitleaks labels the replacement
Dropbox app key as `dropbox-api-token`, but the Dropbox console and browser
PKCE/Chooser protocol identify it as the public client/app key, not the separate
app secret or an OAuth token. The exception matches only that exact public key.
The existing Google Picker key is restricted to the Picker API and the approved
lumafoil.com/localhost referrers; its exact exception and the retired public
Dropbox key remain for history scanning. No generic pattern or secret-bearing
...
| Q9  | ~~Sender domain for Email Sending.~~ **Resolved 2026-09-06.** Email Sending enabled on `lumafoil.com` (`wrangler email sending enable`); Cloudflare created the `cf-bounce` MX/SPF, DKIM and `_dmarc` records under that subdomain only, so the apex domain's mail is untouched. Sender is `no-reply@lumafoil.com`; delivery verified against the live Worker. Alternatives considered: the already-authorized `projects.blowmoney.net`, or the apex (would add `_dmarc.blowmoney.net` `p=reject`). | —                                                                                             |
...
- `blowmoney.net` hosts other properties besides this app. Cookies are scoped
  to `lumafoil.com` (no `Domain` attribute), HSTS is set on the
...
- **Goal:** production deployment at `lumafoil.com`, admin console, hardening, documentation. (The `production` wrangler environment and the first deployment were pulled forward to 2026-09-06, right after M1; see CHANGELOG.)
...
- **Certification checklist:**
  ...
  - [x] 1.3.0 tagged, deployed, released (live at lumafoil.com, 2026-09-07)
...
- **Certification checklist:**
  ...
  - [x] 1.4.0 tagged, deployed, released (live at lumafoil.com, 2026-09-07)
...
- **Owner clarification (2026-09-08):** offline photos, gallery saves, and preset
  ...
  shell caching alone does not satisfy it. Current scan and failed baseline gates
  are recorded in `docs/verification/m19/readiness.md`. Project-owned processes
  may be stopped after identity verification; other projects must be untouched.
- **Visual scope (2026-09-08):** Randy also requested a landing-page and app UI/UX
  polish pass, rejected purple, and suggested `#C86B82`. The design uses rose,
  warm neutrals and charcoal, with accessible darker rose for button text. The
  direction and final visual acceptance scope are in
  `docs/verification/m19/design.md`; production readiness and offline sync remain
  required alongside this work.

- **Scope:** paint before JavaScript (static skeleton with an inline hashed theme script; pre-rendered public pages); render signed-in pages from a persisted query cache and a single bootstrap request; JavaScript diet with a bundle-budget gate in `quality`; an offline-capable service worker with an update flow and account-scoped sign-out cleanup (the original global wipe policy was superseded on 2026-09-09); dependency refresh; request ids, client error reporting and a cron health check on the admin console (no dashboard); a11y/security re-audit; competitor re-check with a "Lumafoil" column in the matrix. Budgets in §5.5 rise to mobile ≥ 90 / desktop ≥ 95 with numeric FCP/LCP/CLS/TBT limits.
- **Certification checklist:**
...
<locale>.html`. Verified: 12 localized static pages, **largest 4.2 kB gzip,
  ...
  - **§4 offline (service worker done; offline-editor journey with §2).** A
    ...
    sign-out the original Worker sent `Clear-Site-Data: "cache", "storage"`.
    **Superseded 2026-09-09:** global clearing can erase a newer account's queue
    when an old logout response arrives late. The Worker now revokes sessions
    with API `no-store` and no global wipe header. Client cleanup is awaited,
    scoped to the captured departing account and serialized with account
    activation; public SW assets contain no private data. Server/header and
    delayed-response/two-account browser checks gate this correction. The full
    offline-**editor** e2e journey needs the §2 persisted
  ...
  - **§2 persister + non-blocking boot (done 2026-09-08, redesigned).** Landed
    ...
    originally 24 h expiry, installed from `main.tsx`) so a return visit and offline boot
    ...
    offline.
  - **2026-09-09 sustained-outage correction:** the original 24-hour display-cache expiry is superseded. Structurally valid snapshots remain quarantined behind live identity validation on connected navigation. When the browser reports Wi-Fi but the Internet is unavailable, only transport failure (TypeError or API status zero) admits the same prepared owner, checked against the captured account generation. Explicit sign-out or HTTP 401/403 clears display admission; HTTP 500 and validation failures never fall back. Future-dated/malformed snapshots still fail. Older-than-24-hour tests independently prove transport-outage success and live-revocation refusal. This implements the user's sustained offline requirement without treating cached display data as server authorization.
  - _(Superseded)_ First §2 attempt (persister + non-blocking boot) hit a
    ...
    **organization list** from a stale cache after a mutation (e.g. just after
    ...
    follow-up. §4's SW shipped independently; its historical global logout wipe
    was superseded by the account-scoped cleanup correction above on 2026-09-09.
...
scan copies may mask that retired value only after original-byte private-value
and archive checks; working-tree, index and built artifacts get no exception.
The independent publication scanner bypasses generic repository ignore files.
This preserves useful history after revocation without permitting a current
credential or another historical finding. Revisit only if the corresponding
history is deliberately removed. See
[the remediation evidence](docs/verification/m19/picker-key-rotation.md).
...
| `vitest.config.ts`                                              | Coverage excludes only test/support files, generated route data, the application/auth/Zod startup seams, dedicated engine/video Worker entries, and workerd-only database/service bindings.                                                                                                                                                                                                                                                                                                                         | M19 counter evidence disproved the old claim that browser coverage cannot instrument canvas, provider, PDF, locale and main-thread video code. Those exclusions were removed on 2026-09-09. Executable coverage includes TypeScript/TSX; raw JSON/CSS remain asset validation and rendered-UI concerns. Existing global 90/90/90/85 floors are unchanged. Remaining startup seams have browser/build proof; dedicated Worker and D1 binding paths have actual browser-message/workerd integration proof. | Retire each remaining exception when its separate execution context becomes instrumentable; never exclude ordinary untested code. |
...
| `scripts/vendor-fonts.py` HTTPS download                        | `nosemgrep: python.lang.security.audit.httpsconnection-detected.httpsconnection-detected` on the connection call only                                                                                                                                                                                                                                                                                                                                                                                               | The rule flags every HTTPSConnection call. The helper supplies `ssl.create_default_context()`, enforces exact HTTPS hosts, rejects credentials/ports/redirects, and has positive/negative tests that assert hostname and certificate verification. Primary rule and Python documentation are linked in `docs/verification/m19/creative-library.md`.                                                                                                                                                      | Remove when the rule recognizes an explicit verified TLS context or the canonical downloader changes.                             |
| `scripts/vendor-stickers.py` HTTPS download                     | `nosemgrep: python.lang.security.audit.httpsconnection-detected.httpsconnection-detected` on the connection call only                                                                                                                                                                                                                                                                                                                                                                                               | Same audited TLS boundary as the font downloader; required fixed GitHub hosts only. Default certificate trust remains enforced and redirects are refused. No global scanner configuration is disabled.                                                                                                                                                                                                                                                                                                   | Remove when the rule recognizes an explicit verified TLS context or the canonical downloader changes.                             |
| `scripts/vendor-stickers.py` Git blob identity                  | `nosemgrep: python.lang.security.insecure-hash-algorithms.insecure-hash-algorithm-sha1` on the Git object calculation only                                                                                                                                                                                                                                                                                                                                                                                          | Git's pinned tree supplies SHA-1 object names, so replacing this algorithm would break identity comparison. `usedforsecurity=False` states its purpose; SHA-256 still records downloaded asset integrity. A fixed independent `git hash-object` oracle tests the calculation; no SHA-1 test suppression exists. See the creative-library evidence and linked Git format documentation.                                                                                                                   | Remove if the source repository migrates to SHA-256 object names or this Git identity comparison is retired.                      |
| `eslint.config.mjs` generated task files                        | `temp/**` directory exclusion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Isolated test state, disposable red-drill checkouts and scanner mirrors belong under the ignored temporary directory. Linting repeated repo mirrors exhausted the 4 GiB Node heap. Canonical source remains outside this directory, and the independent publication gate forbids publishing it.                                                                                                                                                                                                          | Retain while tooling uses this generated directory; never place product source there.                                             |
